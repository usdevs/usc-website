...
export function formatMonthEvents(firestore) {
  ...
  if (
    !isLoaded(eventsStartInMth) ||
    !isLoaded(eventsEndInMth) ||
    !eventsStartInMth ||
    !eventsEndInMth
  ) {
    return null
...
export function formatEventsByDate(firestore, hideSpaceOnly = false) {
  ...
  _.forOwn(allEvents, function(Event, eventID) {
    ...
    for (var day = 0; day <= noOfDays; day++) {
      ...
      if (noOfDays === 0) {
        _.mergeWith(
          eventsByDate,
          { [initialStartDate.toString()]: [newEvent] },
          eventCombiner
        )
      } else {
        ...
        _.mergeWith(
          eventsByDate,
          { [tempStartDate.toString()]: [newEvent] },
          eventCombiner
        )
      }
    }
  ...
  return _.mapValues(eventsByDate, dayEvents =>
    _.sortBy(dayEvents, 'startDate')
  )
}

//Arrange events into arrays of events with a key of the date
export function formatEventsByDateTimeAndVenue(
  firestore,
  ignoreStandardSpaces = true
) {
  ...
  return _.mapValues(eventsByDate, dayEvents => {
    ...
    _.forEach(dayEvents, event => {
      ...
      const noOfTimeslots =
        ((duration.days() * 24 + duration.hours()) * 60 + duration.minutes()) /
        30
      ...
      for (var timeslot = 0; timeslot <= noOfTimeslots; timeslot++) {
        const tempStartTime = event.startDate
          .clone()
          .add(config.timeInterval * timeslot, 'minutes')
          .seconds(0)

        if (timeslot < noOfTimeslots) {
          _.merge(eventsByDateTime, {
          ...
          _.merge(eventsByDateTime, {
            [tempStartTime.toString()]: { blank: true }
          })
        }
      }
    })
...
export function eventTimeDisplay(event, selectedDate) {
  if (event.fullDay) {
    ...
    return (
      event.startDate.format('hh:mm a') +
      ' - ' +
      event.endDate.format('hh:mm a')
    )
  } else {
