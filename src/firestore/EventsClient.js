...
export function formatFirestoreEvent(event) {
  if (
    ...
    !(
      ...
      event.regLink.startsWith('https://')
    )
  ) {
    event = {
      ...event,
...
export function getEventTypes(firestore) {
  firestore.get({ collection: 'eventTypes', orderBy: ['name'] })
}
...
export function getEvents(
  ...
  firestore.get(query).then(() => callback())
...
export function watchEvents(firestore) {
  firestore.setListeners([{ collection: 'events' }])
}
