...
class DaySpaceCalendar extends Component {
    ...
    if (!eventTypes.isLoaded || !spaces.isLoaded) {
      return (
        <h3 style={{ fontWeight: 300 }}>
          <FontAwesomeIcon icon="spinner" spin /> Loading bookings...
        </h3>
      )
    } else if (timeslots) {
      return (
        <div>
          <Container className="d-block d-md-none">
            <Row>{this.spaceBookingDisplayCols(4)}</Row>
          </Container>
          <Container className="d-none d-md-block">
            <Row>{this.spaceBookingDisplayCols(6)}</Row>
          </Container>
          {this.eventModals()}
        </div>
      )
  ...
  spaceBookingDisplayCols = noOfColumns => {
    ...
    _.forEach(
      _.chunk(listOfTimeslots, (12 - colSize) / colSize),
      timeslotChunk => {
        index += 1
        ...
        _.forEach(timeslotChunk, timeslotString => {
          timeslotsCol.push(
            <Col
              xs={colSize}
              key={timeslotString + index}
              className="d-flex justify-content-center"
            >
              <small>{events[timeslotString].displayText}</small>
            </Col>
          )
        })

        timeslotsCol.push(
          <div key={'timeslotDivider' + index} className="w-100"></div>
        )

        _.forEach(venuesUsed, venueID => {
          timeslotsCol.push(
            <Col xs={colSize} key={venueID + index}>
              <small>
                {spaces.data[venueID] ? spaces.data[venueID].shortName : ''}
              </small>
            </Col>
          )
          ...
          _.forEach(timeslotChunk, timeslotString => {
            if (events[timeslotString] && events[timeslotString][venueID]) {
              ...
              timeslotsCol.push(
                <Col
                  xs={colSize}
                  key={venueID + timeslotString}
                  className={
                    (spaceEvent.isStart ? 'rounded-left ' : '') +
                    (spaceEvent.isEnd ? 'rounded-right' : '')
                  }
                  style={{
                    backgroundColor: spaces.data[venueID]
                      ? spaces.data[venueID].colour
                      : ''
                  }}
                  onClick={() =>
                    this.modals[
                      events[timeslotString][venueID].event.id
                    ].toggle()
                  }
                />
              )
              ...
              timeslotsCol.push(
                <Col xs={colSize} key={venueID + timeslotString} />
              )
            }
          })
          ...
          timeslotsCol.push(
            <div
              key={venueID + 'timeslotDivider' + index}
              className="w-100"
            ></div>
          )
        })
    ...
    return (
      <div>
        <h2 style={{ fontWeight: 300 }}>
          <small>{selectedDate.format('Do MMMM')}</small>
