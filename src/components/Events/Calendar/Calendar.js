...
class Calendar extends Component {
  ...
  days() {
    ...
    for (let day of mthDisplayRange.by('day')) {
      ...
      const dayEvents =
        events.isLoaded && events.ordered[day.toString()]
          ? events.ordered[day.toString()]
          : []
      ...
      days.push(
        <Col
          ...
          onClick={() => this.changeSelectedDate(day)}
          className={
            'embed-responsive embed-responsive-1by1 d-flex justify-content-center ' +
            (!belongsToSameMonth ? 'text-muted' : '')
          }
        >
          <Container>
            <Row>
                      ...
                      _.forOwn(venues, (venue, venueID) => {
                        tags.push(
                          <FontAwesomeIcon
                            className="inline-block"
                            icon="circle"
                            color={spaces.data[venue].colour}
                            key={venueID}
                          />
                        )
                      })
  ...
  dayHeaders() {
    ...
    for (let day of wkRange.by('day')) {
      dayHeaders.push(
        <Col key={day.format('YYYYMMDD')} className="dayHeader p-0">
          {day.format('dd')}
        </Col>
      )
    }
  ...
  monthDisplay() {
    ...
    const previousButton = (
      <Button color="link" onClick={() => this.previousMonth(selectedDate)}>
        <FontAwesomeIcon icon="arrow-circle-left" size="3x" />
      </Button>
    )
    const nextButton = (
      <Button color="link" onClick={() => this.nextMonth(selectedDate)}>
        <FontAwesomeIcon icon="arrow-circle-right" size="3x" />
      </Button>
    )
  ...
  render() {
    return (
      <Container>
        {this.monthDisplay()}
        <Row className="text-center">{this.dayHeaders()}</Row>
