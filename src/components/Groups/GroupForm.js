...
class InterestGroupForm extends Component {
      ...
      <Col xs="12" md="6" key="addremove">
        <div className="d-flex justify-content-center">
          ...
          <Button
            outline
            color="primary"
            onClick={() => formApi.setValue('noOfMembers', noOfMembers + 1)}
          >
            Add Member
          </Button>
        </div>
  ...
  typeValidate = (formApi, value) => {
    ...
    formApi.setValue(
      'category',
      groupTypes.data[value] ? groupTypes.data[value].category : null
    )
  ...
  render() {
    ...
    const isInterestGroup = initialValues
      ? initialValues.category === config.categoryIDs.ig
      : forInterestGroup
      ? true
      : false
          ...
          {({ formApi }) => (
            <div>
              <DropdownInput
                field="status"
                hidden={true}
                options={_.flatMap(groupStatuses, status => {
                  return { id: status.id, display: status.name }
                })}
              />
              <TextInput field="category" hidden={true} />
              <h3>Name</h3>
              <TextInput
                field="name"
              ...
              <p>
                <small>(at least {config.minimumIGMembers})</small>
              </p>
              <TextInput field="noOfMembers" hidden={true} />
              <Container className="mb-3">
                <Row>{this.showMembers(formApi)}</Row>
              </Container>
