  ...
  addMember = () => {
    ...
    getUserByEmail(firestore, email, snapshot => {
      if (!snapshot.empty) {
        ...
        setError(
          'User does not exist! Please check user has registered on this site!'
        )
        this.setState({
          error:
            'User does not exist! Please check user has registered on this site!',
  ...
  render() {
    ...
    return (
      <Card className="p-3 mb-3" outline color="primary">
        <h5 className="mb-0" style={{ color: 'dodgerblue' }}>
        ...
        <InputGroup>
          ...
          {fieldState.error || error ? (
            <FormFeedback>
              {error ? error : errortext ? errortext : fieldState.error}
            </FormFeedback>
          ) : null}
        </InputGroup>
      </Card>
