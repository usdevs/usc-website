...
class EventForm extends Component {
  ...
  renderSuggestion = suggestion => (
    <span className="suggestion-content list-unstyled">
      <ModuleCard module={suggestion} hideButtons hasShortDescription />
    </span>
  )
  ...
  submitBtn = () => {
    ...
    const noErrors = _.every(_.values(errors), function(v) {
      return !v
    })
    ...
    if (!noErrors) {
      ...
      buttonOnSubmit(
        review,
        () => this.resetForm(),
        review => this.clearSubmitting(review)
      )
    }
  }
  ...
  displaySemesterOptions = () => {
    ...
    for (var i = 0; i <= -config.reviewStartYear; i++) {
      ...
      semesterOptions.push(
        <option value={semString1} key={semString1}>
          {semString1}
        </option>
      )
      semesterOptions.push(
        <option value={semString2} key={semString2}>
          {semString2}
        </option>
      )
    }
  ...
  render() {
    ...
    const renderInputComponent = inputProps => (
      <div>
        <Input invalid={errors.module} type="text" {...inputProps} />
        {errors.module ? <FormFeedback>Select the module.</FormFeedback> : ''}
      </div>
    )
    ...
    return (
      <Form className="m-3">
        <FormGroup>
          ...
          {moduleData ? (
            <ModuleCard module={moduleData} hideButtons hasShortDescription />
          ) : (
            <Autosuggest
        ...
        <FormGroup>
          ...
          <Input
            ...
            onChange={event =>
              this.handleFormChange(event.target.value, 'semester')
            }
            value={semester}
          >
        ...
        <FormGroup>
          ...
          <Input
            ...
            value={review}
            onChange={event =>
              this.handleFormChange(event.target.value, 'review')
            }
          ...
          <FormGroup check>
            <Label check>
              <Input
                type="checkbox"
                onChange={event =>
                  this.handleFormChange(event.target.value, 'isAnon')
                }
                checked={isAnon}
              />{' '}
              Submit Anonymously (will not display name)
            </Label>
          </FormGroup>
        </FormGroup>
        <Button
        ...
        >
          {formSubmitting ? <FontAwesomeIcon icon="spinner" spin /> : ''}{' '}
          {buttonText}
        </Button>
        <br />
        {submitFailure ? (
